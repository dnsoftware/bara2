...
/**
 * This is the model class for table "{{notice_images}}".
 *
 * The followings are the available columns in table '{{notice_images}}':
 ...
 */
class NoticeImages extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('n_id, filename, titul_tag, fotonumber', 'required'),
			array('n_id, titul_tag, fotonumber', 'numerical', 'integerOnly'=>true),
			array('filename', 'length', 'max'=>256),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('ni_id, n_id, filename, titul_tag, fotonumber', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return NoticeImages the static model class
	 */
