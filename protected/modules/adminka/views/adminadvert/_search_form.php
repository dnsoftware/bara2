<form id="form_filter" method="get" action="<?= Yii::app()->createUrl('/adminka/adminadvert/index');?>" >
    ...
    <table  style="display: inline; width: 100%;">
        <tr style="background-color: #eee;">
            <td style="border: #000020 solid 0px; padding: 0px 0px 0px 5px;">
                <select class="filterselect fchange" name="mainblock[r_id]" id="r_id" style="margin: 0px; width: 250px;">
    ...
    <table>
        <tr>
            <td>
                <select class="filterselect fchange" name="mainblock[old_r_id]" id="old_r_id" style="margin: 0px; width: 250px;">
                    <option value="">--- выберите старую категорию  ---</option>
                    <?
                    foreach ($rub_old_array as $rkey=>$rval)
                    {
                        $selected = " ";
                        if($rkey == intval($_GET['mainblock']['old_r_id']))
                        {
                            $selected = " selected ";
                        }
                        ?>
                        <option disabled <?= $selected;?> style="color:#000; font-weight: bold;" value="<?= $rval['parent']->r_id;?>"><?= $rval['parent']->name;?></option>
                        <?
                        foreach ($rval['childs'] as $ckey=>$cval)
                        {
                            $selected = " ";
                            if($ckey == intval($_GET['mainblock']['old_r_id']))
                            {
                                $selected = " selected ";
                            }
                            ?>
