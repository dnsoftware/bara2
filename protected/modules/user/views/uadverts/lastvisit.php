...
<h1 style="font-size: 18px; margin-top: 15px; margin-left: 10px;">Недавно просмотренные объявления</h1>
...
<tr>
    ...
    if(count($parent_rubriks) > 0)
    {
        ...
        ?>
        <tr>
                <td><a href="/user/lastvisit/<?= $pval->transname;?>/"><?= $pval->name;?></a></td> <td><?= $parent_ids_count[$pval->r_id];?></td>
        </tr>
        <?
            if(count($subrub_array) > 0)
            {
                ...
                ?>
                <tr>
                    <td style="padding-left: 20px;"><a href="/user/lastvisit/<?= $pval->transname;?>/<?= $sval->transname;?>/"><?= $sval->name;?></a></td> <td><?= $rub_counter[$sval->r_id];?></td>
                </tr>
                <?
                }
            }
    ...
    if(count($useradverts) > 0)
    {
        ...
        <tr id="row_favorit_<?= $uval->n_id;?>">
            ...
            <td style="border-left: #ddd solid 1px;  border-top: #ddd solid 1px; ">
                <?= date("d-m-Y H:i:s", $lastvisitdate[$uval->n_id]['date_view']);?>
            </td>
        </tr>
        <?
        }
